...
<h1>Trade</h1><div class="trade-grid"><section class="panel">
...
@if(quote();as q){<div class="quote"><div><small>Bid</small><strong>{{q.bid|number:'1.2-8'}}</strong></div><div><small>Ask</small><strong>{{q.ask|number:'1.2-8'}}</strong></div></div><p><small>Trade currency</small> <strong>{{q.quoteCurrency}}</strong> · <small>Available cash</small> <strong>{{availableCash(q.quoteCurrency)|number:'1.2-8'}} {{q.quoteCurrency}}</strong></p>}
...
<label>Quantity</label><input type="number" min="0.00000001" step="any" [(ngModel)]="quantity"><button class="primary" (click)="submit()" [disabled]="busy()">{{busy()?'Submitting…':'Submit order'}}</button>
</section>
@if(result();as r){<section class="panel success"><h2>Order accepted</h2><p><strong>{{r.side}} {{r.quantity|number:'1.0-8'}} {{symbol}}</strong></p><p>Status: {{r.status}}</p><p>Order: <strong>{{shortId(r.orderId)}}</strong></p></section>}
