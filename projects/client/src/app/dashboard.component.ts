...
@Component({
 selector:'app-dashboard',standalone:true,imports:[DecimalPipe,DatePipe,FormsModule],
 template:`
 <h1>Portfolio</h1>
 <section class="cards"><article><small>Cash currencies</small><strong>{{cash().length}}</strong></article><article><small>Positions</small><strong>{{positions().length}}</strong></article><article><small>Orders</small><strong>{{orders().length}}</strong></article></section>
 <h2>Cash wallet</h2>
 <table><thead><tr><th>Currency</th><th>Balance</th><th>Updated</th></tr></thead><tbody>@for(c of cash();track c.accountId+c.currency){<tr><td>{{c.currency}}</td><td>{{c.balance|number:'1.2-8'}}</td><td>{{c.updatedAt|date:'medium'}}</td></tr>}</tbody></table>
 <div class="trade-grid">
  <section class="panel"><h3>Deposit / withdraw</h3><label>Account</label><select [(ngModel)]="accountId" (ngModelChange)="loadTransactions()">@for(a of accounts();track a.accountId){<option [value]="a.accountId">{{a.accountNumber}}</option>}</select><label>Currency</label><input [(ngModel)]="currency" maxlength="10"><label>Amount</label><input type="number" min="0.01" step="any" [(ngModel)]="amount"><div class="side"><button class="primary" (click)="deposit()" [disabled]="busy()">Deposit</button><button (click)="withdraw()" [disabled]="busy()">Withdraw</button></div></section>
 ...
 @if(message()){<p>{{message()}}</p>} @if(error()){<section class="panel error"><p>{{error()}}</p></section>}
 <h3>Cash history</h3><table><thead><tr><th>Time</th><th>Type</th><th>Currency</th><th>Amount</th></tr></thead><tbody>@for(t of transactions();track t.transactionId){<tr><td>{{t.createdAt|date:'medium'}}</td><td>{{t.type}}</td><td>{{t.currency}}</td><td>{{t.amount|number:'1.2-8'}}</td></tr>}</tbody></table>
 <h2>Positions</h2><table><thead><tr><th>Symbol</th><th>Type</th><th>Quantity</th></tr></thead><tbody>@for(p of positions();track p.instrumentId){<tr><td>{{p.symbol}}</td><td>{{p.instrumentType}}</td><td>{{p.quantity|number:'1.0-8'}}</td></tr>}</tbody></table>
 <h2>Recent orders</h2><table><thead><tr><th>Order</th><th>Side</th><th>Quantity</th><th>Status</th><th>Submitted</th></tr></thead><tbody>@for(o of orders();track o.orderId){<tr><td>{{shortId(o.orderId,'ORD')}}</td><td>{{o.side}}</td><td>{{o.quantity|number:'1.0-8'}}</td><td>{{o.status}}</td><td>{{o.submittedAt|date:'medium'}}</td></tr>}</tbody></table>`
})
export class DashboardComponent {
 ...
 shortId(id:string,prefix:string){return `${prefix}-${id.replaceAll('-','').slice(0,8).toUpperCase()}`;}
 private start(){this.error.set('');this.message.set('');this.busy.set(true);} private finish(message:string){this.message.set(message);this.busy.set(false);this.refresh();this.loadTransactions();} private fail(e:any){this.error.set(e.error?.message??'Cash operation failed');this.busy.set(false);}
}
